...
class UpdateWarehouseShippingRequest extends FormRequest
{
    ...
    {
        $id = $this->route('id') ?? $this->route('shipping');
        if (is_object($id)) {
            $id = $id->id;
        }

        return [
            'shipping_no' => 'required|string|max:255|unique:warehouse_shippings,shipping_no,' . $id,
