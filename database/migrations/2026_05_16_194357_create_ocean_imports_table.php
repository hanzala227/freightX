...
return new class extends Migration
{
    ...
    {
        Schema::create('ocean_imports', function (Blueprint $table) {
            $table->id();
            $table->string('file_no')->unique(); // MOI-25040001
            $table->string('mbl_no')->nullable();
            $table->date('post_date')->nullable();
            
            // Core Parties
            $table->foreignId('office_id')->nullable()->constrained('offices');
            $table->foreignId('op_id')->nullable()->constrained('users'); // Operator
            ...
            $table->foreignId('acct_carrier_id')->nullable()->constrained('trade_partners'); // Accounting Carrier
            ...
            $table->string('ship_mode')->default('FCL'); // FCL, LCL
            
            // Logistics
            $table->foreignId('vessel_id')->nullable()->constrained('vessels');
            $table->string('voyage')->nullable();
            $table->foreignId('pol_id')->nullable()->constrained('ports'); // Port of Loading
            $table->foreignId('pod_id')->nullable()->constrained('ports'); // Port of Discharge
            $table->foreignId('del_id')->nullable()->constrained('ports'); // Place of Delivery
            $table->foreignId('fdest_id')->nullable()->constrained('ports'); // Final Destination
            $table->foreignId('receipt_id')->nullable()->constrained('ports'); // Place of Receipt
            ...
            $table->string('freight_term')->nullable(); // PREPAID, COLLECT
            $table->string('obl_type')->nullable(); // ORIGINAL, SEAWAY
    ...
    {
        Schema::dropIfExists('ocean_imports');
    }
};
