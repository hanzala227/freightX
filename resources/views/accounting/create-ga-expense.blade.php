<x-layout>
    @php
        $editMode = isset($ga_expense) && $ga_expense->exists;
        $linesData = [];
        if ($editMode && $ga_expense->lines->count() > 0) {
            foreach ($ga_expense->lines as $line) {
                $linesData[] = [
        ...
        $docData = [];
        if ($editMode && $ga_expense->documents) {
            $docData = $ga_expense->documents->map(fn($d) => [
    ...
    <div class="page-content" x-data="gaExpenseForm()">
        <form id="gaExpenseForm" action="{{ $editMode ? route('accounting.ga-expense.update', $ga_expense->id) : route('accounting.ga-expense.store') }}" method="POST">
            @csrf
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li><a href="{{ route('accounting.ga-expense.index') }}">Accounting</a> <i class="fa fa-angle-right"></i></li>
                    <li><a href="{{ route('accounting.ga-expense.index') }}">G&A Expense</a> <i class="fa fa-angle-right"></i></li>
                    <li><span style="color:#333;font-weight:700;">{{ $editMode ? 'Edit: ' . ($ga_expense->invoice_no ?? '') : 'New Entry' }}</span></li>
                </ul>
            </div>
            ...
            <div class="portlet light">
                <div class="portlet-title" style="cursor:pointer;">
                    <span class="caption-subject"><i class="fa fa-file-text-o"></i> Expense Details</span>
                </div>
                <div class="portlet-body">
                    <div class="form-grid-4">
                        <!-- Column 1 -->
                        <div>
                            <div class="form-group-gf"><label class="form-label-gf"><span style="color:red;">*</span> Vendor</label>
                            ...
                            <div class="form-group-gf"><label class="form-label-gf">Vendor Invoice No.</label>
                                <div class="form-input-container">
                                    <input type="text" name="vendor_invoice_no" class="form-control-gf" value="{{ old('vendor_invoice_no', $ga_expense->vendor_invoice_no ?? '') }}">
                                </div>
                            </div>
                            <div class="form-group-gf"><label class="form-label-gf">File No.</label>
                                <div class="form-input-container">
                                    <input type="text" name="file_no" class="form-control-gf" value="{{ old('file_no', $ga_expense->file_no ?? '') }}" placeholder="Optional">
                                </div>
                            </div>
                        </div>
                        <!-- Column 2 -->
                        ...
                        <div>
                            <div class="form-group-gf"><label class="form-label-gf"><span style="color:red;">*</span> Amount</label>
                                <div class="form-input-container">
                                    <input type="number" step="0.01" name="total_amount" class="form-control-gf" style="text-align:right;" x-model="form.total_amount" @input="recalcSubtotal()" required>
                                </div>
                            </div>
                            <div class="form-group-gf"><label class="form-label-gf">Tax Amount</label>
                                <div class="form-input-container">
                                    <input type="number" step="0.01" name="tax_total" class="form-control-gf" style="text-align:right;" x-model="form.tax_total" @input="recalcSubtotal()">
                                </div>
                            </div>
                            <div class="form-group-gf"><label class="form-label-gf">Subtotal</label>
                                <div class="form-input-container">
                                    <input type="number" step="0.01" name="subtotal" class="form-control-gf" style="text-align:right;" :value="computedSubtotal" readonly>
                                </div>
                            </div>
                            <div class="form-group-gf"><label class="form-label-gf">Discount %</label>
                                <div class="form-input-container">
                                    <input type="number" step="0.01" min="0" max="100" name="discount_pct" class="form-control-gf" style="text-align:right;" x-model="form.discount_pct">
                                </div>
                            </div>
                        </div>
                        <!-- Column 4 -->
                        <div>
                            <div class="form-group-gf"><label class="form-label-gf">Billing Address</label>
                                <div class="form-input-container">
                                    <input type="text" name="billing_address" class="form-control-gf" value="{{ old('billing_address', $ga_expense->billing_address ?? '') }}" placeholder="Optional">
                                </div>
                            </div>
                            <div class="form-group-gf"><label class="form-label-gf">Remark</label>
                                <div class="form-input-container">
                                    <input type="text" name="internal_remark" class="form-control-gf" value="{!! old('internal_remark', $ga_expense->internal_remark ?? '') !!}">
                                </div>
                            </div>
                            <div class="form-group-gf" style="margin-top:6px;">
                                <label class="form-label-gf">Invoice Received</label>
                                <div class="form-input-container">
                                    <label style="display:flex;align-items:center;gap:4px;font-size:10px;color:#475569;cursor:pointer;">
                                        <input type="checkbox" name="invoice_received" value="1" {{ old('invoice_received', $ga_expense->invoice_received ?? '') ? 'checked' : '' }}> Yes
                                    </label>
                                </div>
                            </div>
                        </div>
            ...
            <div class="portlet light">
                <div class="portlet-title">
                    <span class="caption-subject"><i class="fa fa-list"></i> Expense Lines</span>
                </div>
                <div class="portlet-body">
                    <div style="display:flex;gap:4px;margin-bottom:8px;align-items:center;">
                        <button type="button" class="btn-tool-icon" style="background:#22c55e;color:#fff;border-color:#16a34a;" @click="addLine()"><i class="fa fa-plus"></i></button>
                    ...
                    <div class="table-responsive">
                        <table class="table-custom">
                            <thead>
                                <tr>
                                    <th style="width:28px;"><input type="checkbox" @change="toggleAllLines($event.target.checked)"></th>
                            ...
                            <tbody>
                                <template x-for="(line, idx) in lines" :key="idx">
                                    <tr>
                                        ...
                                        <td><input type="number" step="0.01" min="0" class="form-control-gf" style="height:18px;font-size:9px;text-align:right;" x-model="line.qty" @input="calcLineAmount(idx)"></td>
                                        <td><input type="number" step="0.01" min="0" class="form-control-gf" style="height:18px;font-size:9px;text-align:right;" x-model="line.rate" @input="calcLineAmount(idx)"></td>
                                        <td style="text-align:right;font-weight:600;color:#3b82f6;" x-text="formatAmount(line.amount)"></td>
                                    </tr>
                                </template>
                                <template x-if="lines.length === 0">
                                    <tr><td colspan="9" style="text-align:center;padding:20px;color:#94a3b8;font-style:italic;"><i class="fa fa-inbox" style="font-size:20px;display:block;margin-bottom:4px;"></i>No expense lines. Click <span style="color:#32c5d2;cursor:pointer;" @click="addLine()">+ Add Line</span></td></tr>
                                </template>
                                <tr class="total-row" x-show="lines.length > 0">
                                    <td colspan="7" style="text-align:right;padding-right:15px;color:#3b82f6;">Total</td>
                                    <td style="text-align:right;" x-text="formatAmount(totalAmount)"></td>
                                    <td></td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <div class="action-bar">
                ...
                @endif
                <a href="{{ route('accounting.ga-expense.index') }}" class="btn-tool" target="_blank"><i class="fa fa-arrow-left"></i> CANCEL</a>
            </div>
        </form>
    ...
    <script>
        function gaExpenseForm() {
            return {
                ...
                form: {
                    bill_to_id: '{{ old("bill_to_id", $editMode ? ($ga_expense->bill_to_id ?? "") : "") }}',
                    invoice_no: '{{ old("invoice_no", $editMode ? ($ga_expense->invoice_no ?? "") : "INF-" . date("ymdHis")) }}',
                    invoice_date: '{{ old("invoice_date", $editMode && $ga_expense->invoice_date ? $ga_expense->invoice_date->format("Y-m-d") : date("Y-m-d")) }}',
                    due_date: '{{ old("due_date", $editMode && $ga_expense->due_date ? $ga_expense->due_date->format("Y-m-d") : date("Y-m-d", strtotime("+30 days"))) }}',
                    office_id: '{{ old("office_id", $editMode ? ($ga_expense->office_id ?? "") : "") }}',
                    currency_id: '{{ old("currency_id", $editMode ? ($ga_expense->currency_id ?? "") : "") }}',
                    total_amount: '{{ old("total_amount", $editMode ? ($ga_expense->total_amount ?? "0") : "0") }}',
                    tax_total: '{{ old("tax_total", $editMode ? ($ga_expense->tax_total ?? "0") : "0") }}',
                    discount_pct: '{{ old("discount_pct", $editMode ? ($ga_expense->discount_pct ?? "0") : "0") }}',
                    billing_address: '{!! old("billing_address", $editMode ? ($ga_expense->billing_address ?? "") : "") !!}',
                    internal_remark: '{!! old("internal_remark", $editMode ? ($ga_expense->internal_remark ?? "") : "") !!}',
                },
                ...
                addLine() {
                    this.lines.push({ charge_code: '', description: '', unit: 'UNIT', currency: 'USD', qty: 1, rate: 0, amount: 0, selected: false });
                },
                addLines(count) {
                ...
                validateForm() {
                    let valid = true;
                    let missing = [];
                    if (!this.form.bill_to_id) { missing.push('Vendor'); valid = false; }
                ...
                saveViaAjax() {
                    ...
                    const form = document.getElementById('gaExpenseForm');
                    ...
                    .then(({ status, data }) => {
                        if (data.success) {
                            ...
                            if (this.saveAction === 'save_new') {
                                setTimeout(() => window.location.href = '{{ route("accounting.ga-expense.create") }}', 800);
                ...
                uploadDocument(e) {
                    ...
                    fetch('{{ route("accounting.invoices.documents.store", $ga_expense->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: fd })
