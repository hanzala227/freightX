<x-layout>
        ...
        <form id="shippingForm" action="{{ isset($shipping) ? route('shipping.update', $shipping->id) : route('shipping.store') }}" method="POST">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li><a href="{{ route('shipping.index') }}">Warehouse Shipping</a> <i class="fa fa-angle-right"></i></li>
                    <li><span style="color:#333;font-weight:700;">{{ isset($shipping) ? 'Edit: ' . $shipping->shipping_no : 'New Shipping' }}</span></li>
                </ul>
            </div>
            ...
            <div x-show="activeTab === 'basic'" class="main-grid">
                <div class="portlet light">
                    <div class="portlet-title" style="cursor:pointer;">
                        <span class="caption-subject"><i class="fa fa-truck"></i> Shipping Information</span>
                    </div>
                    <div class="portlet-body">
                        <div class="form-grid-4">
                            <!-- Column 1 -->
                            <div>
                                <div class="form-group-gf"><label class="form-label-gf"><span style="color:red;">*</span> Shipping No.</label>
                                    <div class="form-input-container">
                                        <input type="text" name="shipping_no" class="form-control-gf" x-model="form.shipping_no" required>
                                    </div>
                                </div>
                                <div class="form-group-gf"><label class="form-label-gf"><span style="color:red;">*</span> Post Date</label>
                                    <div class="form-input-container">
                                        <input type="date" name="shipping_date" class="form-control-gf" x-model="form.shipping_date" required>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-input-container">
                                        <select name="office_id" class="form-control-gf" x-model="form.office_id" required>
                                            ...
                                            @foreach($offices as $o)
                                                <option value="{{ $o->id }}" {{ (isset($shipping) && $shipping->office_id == $o->id) ? 'selected' : '' }}>{{ $o->code }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <div class="form-group-gf"><label class="form-label-gf"><span style="color:red;">*</span> Warehouse</label>
                                    <div class="form-input-container">
                                        <select name="warehouse_id" class="form-control-gf" x-model="form.warehouse_id" required>
                                            <option value="">Select Warehouse...</option>
                                            @foreach($warehouses as $w)
                                                <option value="{{ $w->id }}" {{ (isset($shipping) && $shipping->warehouse_id == $w->id) ? 'selected' : '' }}>{{ $w->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group-gf"><label class="form-label-gf">Ship To</label>
                                    <div class="form-input-container">
                                        <x-inline-select name="ship_to" :options="$tradePartners" module="trade-partner" x-model="form.ship_to" class="form-control-gf" />
                                    </div>
                                </div>
                            </div>
                            <!-- Column 3 -->
                            <div>
                                ...
                                <div class="form-group-gf"><label class="form-label-gf">Truck B/L No.</label>
                                    <div class="form-input-container">
                                        <input type="text" name="truck_bl_no" class="form-control-gf" x-model="form.truck_bl_no" placeholder="Truck B/L No.">
                                    </div>
                                </div>
                                <div class="form-group-gf"><label class="form-label-gf">Order No.</label>
                                    <div class="form-input-container">
                                        <input type="text" name="order_no" class="form-control-gf" x-model="form.order_no" placeholder="Order No.">
                                    </div>
                                </div>
                            ...
                            <!-- Column 4 -->
                            <div>
                                <div class="form-group-gf"><label class="form-label-gf"><span style="color:red;">*</span> Out Date</label>
                                    <div class="form-input-container">
                                        <input type="date" name="out_date" class="form-control-gf" x-model="form.out_date" required>
                                    </div>
                                </div>
                                ...
                                <div class="form-group-gf"><label class="form-label-gf">Status</label>
                                    <div class="form-input-container">
                                        <select name="status" class="form-control-gf" x-model="form.status">
                                            <option value="Pre-Shipping">Pre-Shipping</option>
                            ...
                            <table class="table-custom">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Order P.O. No.</th>
                                        <th style="width:60px;text-align:right;">Shipping Qty</th>
                                        ...
                                        <th style="width:100px;">B/L Inventory</th>
                                        <th style="width:100px;">Date</th>
                                        <th style="width:120px;">B/L | Bkg. No.</th>
                                ...
                                <tbody>
                                    <template x-for="(item, idx) in items" :key="idx">
                                        <tr>
                                            ...
                                            <td><input type="number" step="0.01" class="form-control-gf" style="height:18px;font-size:9px;text-align:right;" x-model="item.shipping_qty"></td>
                                            <td><input type="text" class="form-control-gf" style="height:18px;font-size:9px;" x-model="item.qty_unit" placeholder="Unit"></td>
                                            <td style="text-align:center;" x-text="item.shipping_qty || 0"></td>
                                            ...
                                            <td><input type="text" class="form-control-gf" style="height:18px;font-size:9px;" x-model="item.bl_inventory" placeholder="B/L Loc"></td>
                                            <td><input type="date" class="form-control-gf" style="height:18px;font-size:9px;" x-model="item.item_date"></td>
                                            <td><input type="text" class="form-control-gf" style="height:18px;font-size:9px;" x-model="item.bl_booking_no" placeholder="B/L No."></td>
                                            <td><input type="number" step="0.01" class="form-control-gf" style="height:18px;font-size:9px;text-align:right;" x-model="item.allocated_qty"></td>
                                        </tr>
                                    </template>
                                    ...
                                    <tr class="total-row" x-show="items.length > 0">
                                        ...
                                        <td style="text-align:right;" x-text="itemTotals.shipping_qty"></td>
                                        <td></td>
                                        <td style="text-align:center;" x-text="itemTotals.shipping_qty"></td>
                    ...
                    <div class="portlet-body">
                        ...
                        <div class="table-responsive">
                            <table class="table-custom">
                                ...
                                <tbody>
                                    <tr><td colspan="11" style="text-align:center;padding:30px;color:#94a3b8;font-style:italic;"><i class="fa fa-file-text-o" style="font-size:20px;display:block;margin-bottom:6px;"></i>No invoices or costs yet. Use the buttons above to create.</td></tr>
                                    <tr class="total-row"><td colspan="3" style="text-align:right;padding-right:15px;color:#3b82f6;">Total</td><td style="text-align:right;">0.00</td><td style="text-align:right;">0.00</td><td style="text-align:right;">0.00</td><td colspan="5"></td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
            ...
            <div x-show="activeTab === 'status'" x-cloak>
                                ...
                                <div class="form-group-gf"><label class="form-label-gf">Operator</label>
                                    <div class="form-input-container">
                                        <select name="op_id" class="form-control-gf" x-model="form.op_id">
                                            ...
                                            @foreach($users as $u)
                                                <option value="{{ $u->id }}" {{ (isset($shipping) && $shipping->op_id == $u->id) ? 'selected' : '' }}>{{ $u->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                ...
                <div class="portlet light">
                    ...
                    <div class="portlet-body">
                        @if(isset($shipping) && $shipping->created_at)
                        <ul class="timeline">
                            <li class="timeline-log">
                                <div class="timeline-time">{{ $shipping->created_at->format('m-d-Y') }}<br><span style="font-size:9px;">{{ $shipping->created_at->format('H:i') }}</span></div>
                                <div class="timeline-icon"><i class="fa fa-plus"></i></div>
                                <div class="timeline-body">
                                    <h2>Shipping Record Created</h2>
                                    <div class="timeline-content">{{ $shipping->operator->name ?? 'System' }}</div>
                                </div>
                            </li>
                            @if($shipping->updated_at && $shipping->updated_at->gt($shipping->created_at))
                            <li class="timeline-log">
                                <div class="timeline-time">{{ $shipping->updated_at->format('m-d-Y') }}<br><span style="font-size:9px;">{{ $shipping->updated_at->format('H:i') }}</span></div>
                                ...
                                <tbody>
                                    <template x-for="(doc, idx) in documents" :key="doc.id || idx">
                                        <tr>
                                            ...
                                            <td style="text-align:center;">
                                                <a :href="'/warehouse/shipping/documents/' + doc.id + '/download'" class="btn-tool-icon" style="width:18px;height:18px;font-size:9px;text-decoration:none;" title="Download"><i class="fa fa-download"></i></a>
            ...
            <div class="action-bar">
                <button type="button" class="btn-tool green" @click="handleSubmit('save_close')"><i class="fa fa-save"></i> @if(isset($shipping)) UPDATE @else SAVE @endif</button>
                @if(!isset($shipping))
                <button type="button" class="btn-tool" @click="handleSubmit('save_new')"><i class="fa fa-plus-circle"></i> SAVE &amp; NEW</button>
                @endif
                <a href="{{ route('shipping.index') }}" class="btn-tool" target="_blank"><i class="fa fa-arrow-left"></i> CANCEL</a>
            </div>
        </form>
    ...
    @push('scripts')
    <script>
        function shippingCreateApp() {
            return {
                activeTab: 'basic',
                saveAction: 'save_close',
                isSaved: {{ isset($shipping) ? 'true' : 'false' }},

                form: {
                    shipping_no: '{{ old("shipping_no", isset($shipping) ? $shipping->shipping_no : "WS-" . now()->format("YmdHis")) }}',
                    shipping_date: '{{ old("shipping_date", isset($shipping) && $shipping->shipping_date ? $shipping->shipping_date->format("Y-m-d") : date("Y-m-d")) }}',
                    office_id: '{{ old("office_id", $shipping->office_id ?? "") }}',
                    warehouse_id: '{{ old("warehouse_id", $shipping->warehouse_id ?? "") }}',
                    customer_id: '{{ old("customer_id", $shipping->customer_id ?? "") }}',
                    bill_to_id: '{{ old("bill_to_id", $shipping->bill_to_id ?? "") }}',
                    ship_to: '{{ old("ship_to", $shipping->ship_to ?? "") }}',
                    trucker_id: '{{ old("trucker_id", $shipping->trucker_id ?? "") }}',
                    quotation_no: '{{ old("quotation_no", $shipping->quotation_no ?? "") }}',
                    order_no: '{{ old("order_no", $shipping->order_no ?? "") }}',
                    truck_bl_no: '{{ old("truck_bl_no", $shipping->truck_bl_no ?? "") }}',
                    order_date: '{{ old("order_date", isset($shipping) && $shipping->order_date ? $shipping->order_date->format("Y-m-d") : "") }}',
                    out_date: '{{ old("out_date", isset($shipping) && $shipping->out_date ? $shipping->out_date->format("Y-m-d") : "") }}',
                    status: '{{ old("status", $shipping->status ?? "Pre-Shipping") }}',
                    pallet: '{{ old("pallet", $shipping->pallet ?? "") }}',
                    op_id: '{{ old("op_id", $shipping->op_id ?? "") }}',
                    internal_remark: '{!! old("internal_remark", $shipping->internal_remark ?? "") !!}',
                },
                ...
                init() {
                    @if(isset($shipping) && $shipping->memos_data)
                        this.memos = @json($shipping->memos_data);
                    @endif
                ...
                validateForm() {
                    if (!this.form.shipping_no) { showToast('error', 'Shipping No. is required.'); return false; }
                    if (!this.form.shipping_date) { showToast('error', 'Post Date is required.'); return false; }
                    if (!this.form.office_id) { showToast('error', 'Office is required.'); return false; }
                    if (!this.form.customer_id) { showToast('error', 'Customer is required.'); return false; }
                    if (!this.form.warehouse_id) { showToast('error', 'Warehouse is required.'); return false; }
                    if (!this.form.out_date) { showToast('error', 'Out Date is required.'); return false; }
                    return true;
                },
                ...
                get itemTotals() {
                    const t = { shipping_qty: 0, weight: 0, measure: 0 };
                    this.items.forEach(item => {
                        t.shipping_qty += parseFloat(item.shipping_qty) || 0;
                        t.weight += parseFloat(item.weight_kg) || 0;
                        t.measure += parseFloat(item.measure_cbm) || 0;
                    });
                    return {
                        shipping_qty: t.shipping_qty.toFixed(2),
                ...
                addItem() {
                    this.items.push({ sku_no: '', customer_po: '', description: '', order_po_no: '', shipping_qty: 0, qty_unit: '', weight_kg: 0, measure_cbm: 0, inventory: '', bl_inventory: '', item_date: '', bl_booking_no: '', allocated_qty: 0 });
                },
                ...
                uploadDocument(e) {
                    ...
                    fetch('{{ route("shipping.documents.store", $shipping->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: fd })
                ...
                deleteDocument(id, idx) {
                    ...
                    fetch('/warehouse/shipping/documents/' + id, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })
                ...
                deleteSelectedDocs() {
                    ...
                    [...this.selectedDocIds].sort((a, b) => b - a).forEach(i => {
                        ...
                        if (doc && doc.id) fetch('/warehouse/shipping/documents/' + doc.id, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } });
                ...
                createInvoice() {
                    @if(isset($shipping)) window.open('{{ route("accounting.invoices.create") }}?customer_id={{ $shipping->customer_id ?? "" }}&type=AR&source=shipping', '_blank');
                    @else showToast('info', 'Save first.');
                    @endif
                },
                createDcNote() {
                    @if(isset($shipping)) window.open('{{ route("accounting.invoices.create") }}?customer_id={{ $shipping->customer_id ?? "" }}&type=AP&note=dc&source=shipping', '_blank');
                    @else showToast('info', 'Save first.');
                    @endif
                },
                createCost() {
                    @if(isset($shipping)) window.open('{{ route("accounting.invoices.create") }}?customer_id={{ $shipping->customer_id ?? "" }}&type=AP&note=cost&source=shipping', '_blank');
                    @else showToast('info', 'Save first.');
                    @endif
                },
                    ...
                    .then(({ status, data }) => {
                        if (data.success) {
                            ...
                            if (this.saveAction === 'save_new') {
                                setTimeout(() => window.location.href = '{{ route("shipping.create") }}', 800);
